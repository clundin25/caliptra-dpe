    ...
    fn execute(
        ...
        let (derived_pubkey_x, derived_pubkey_y) = match pub_key {
            ExportedPubKey::Ecdsa(pub_key) => {
                ...
                let derived_pubkey_x: [u8; DPE_PROFILE.ecc_int_size()] = x
                    .get(0..DPE_PROFILE.ecc_int_size())
                    .ok_or(DpeErrorCode::InternalError)?
                    .try_into()
                    .map_err(|_| DpeErrorCode::InternalError)?;
                let derived_pubkey_y: [u8; DPE_PROFILE.ecc_int_size()] = y
                    .get(0..DPE_PROFILE.ecc_int_size())
                    .ok_or(DpeErrorCode::InternalError)?
                    .try_into()
                    .map_err(|_| DpeErrorCode::InternalError)?;
...
mod tests {
    ...
    use crypto::{
        ...
        Crypto, ExportedPubKey, SignatureAlgorithm, SignatureType,
    };
